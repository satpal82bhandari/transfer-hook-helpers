...
console.log("transferFeeConfigAuthority : ",transferFeeConfigAuthority.publicKey, "\n\n");
const withdrawWithheldAuthority = Keypair.generate();
console.log("withdrawWithheldAuthority : ",withdrawWithheldAuthority.publicKey, "\n\n");
...
const calcFee = (transferAmount * BigInt(feeBasisPoints)) / BigInt(10_000); // expect 10 token as fee
const fee = calcFee > maxFee ?  maxFee : calcFee; // expect 9 token as fee
...
async function main() {
    ...
    // Step 2 - Create a New Token
       ...
       console.log("rent lamports : ",mintLamports, "\n\n");
    ...
    const sourceAccount = await createAssociatedTokenAccountIdempotent(connection, payer, mint, sourceWallet.publicKey, {}, TOKEN_2022_PROGRAM_ID);
    ...
    const destinationAccount = await createAssociatedTokenAccountIdempotent(connection, payer, mint, destinationWallet.publicKey, {}, TOKEN_2022_PROGRAM_ID);
    console.log("destination token account",destinationAccount);
    ...
    // --------------------------------------------------------------------------------------------------
        ...
        const feeVault = Keypair.generate();
        const feeVaultAccount = await createAssociatedTokenAccountIdempotent(connection, payer, mint, feeVault.publicKey, {}, TOKEN_2022_PROGRAM_ID);
        console.log("feeVaultAccount : ",feeVaultAccount);
